      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <h1 className="text-lg sm:text-xl hover:text-3xl md:text-2xl font-bold text-blue-600 dark:text-blue-400">
                Shopify
              </h1>
            </Link>
          ...
          {/* Center navigation */}
          <nav className="pl-3 hidden lg:flex items-center text-sm space-x-6">
            {navLinks.map((item) => {
              const isActive =
                item.to === "/"
                  ? currentPath === "/"
          ...
          <div className="flex items-center space-x-2 lg:space-x-4">
            ...
            <Link
              to="/orders"
              className="flex items-center px-1 text-xs sm:px-4 lg:px-6 py-1.5 text-xs sm:text-base lg:text-lg font-medium text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors rounded-md min-w-max whitespace-nowrap"
            >
              <span className="hidden sm:inline">
                Returns <span className="font-extrabold">& Orders</span>
              </span>
              <span className="sm:hidden">Returns & Orders</span>
            </Link>
            <button
              onClick={handleCartToggle}
              className="relative group p-1.5 lg:p-2 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-all duration-500 ease-in-out transform hover:scale-105 active:scale-95"
              aria-label="Shopping cart"
            >
              <ShoppingCart className="h-5 w-5 lg:h-10 lg:w-10 dark:text-white text-black transition-transform duration-500 ease-in-out group-hover:scale-110" />
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 dark:bg-red-600 text-white text-xs rounded-full h-4 w-4 lg:h-5 lg:w-5 flex items-center justify-center transition-all duration-500 ease-in-out animate-pulse">
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-2 text-lg space-y-2">
            {navLinks.map((item) => (
              <Link
                key={item.to}
                to={item.to}
